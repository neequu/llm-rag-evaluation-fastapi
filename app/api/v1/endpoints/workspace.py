...
async def upload_file(
    ...
    return await upload_doc(
        current_user=current_user, db=db, file=file, workspace_id=workspace_id
    )
...
@router.post(
    "/{workspace_id}/documents/batch-upload",
    response_model=list[DocumentRead],
    status_code=status.HTTP_201_CREATED,
)
async def upload_files(
    workspace_id: UUID,
    files: Annotated[list[SwaggerUploadFile], File(description="File to upload")],
    current_user: CurrentUser,
    db: AsyncSession,
):
    res = []
    for f in files:
        t = await upload_doc(
            current_user=current_user, db=db, file=f, workspace_id=workspace_id
        )
        res.append(t)
